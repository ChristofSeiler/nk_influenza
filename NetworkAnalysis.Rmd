...
if(length(V(module)) > 0) {
  ...
  plotModule(module, diff.expr = logFC, labels = node_symbol, layout = layout.kamada.kawai)
...
gene_overlap = t(sapply(all_pathways,function(pathway) {
  genes_one_pathway = pathway$GENE[c(TRUE, FALSE)]
  list(name=pathway$NAME,
       overlap=mean(genes_one_pathway %in% gene_table$X))
  }))
